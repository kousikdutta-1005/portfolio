const images = [
  ...
  { src: "/assets/images/about/philips-3.png", alt: "Good times at Philips Healthcare", caption: "Team at Philips", class: "md:col-span-3 md:row-span-2", type: "pro" },
...
// Shuffle beautifully interleaving pro and personal
const shuffled = [];
let pro = images.filter(i => i.type === 'pro').sort(() => Math.random() - 0.5);
let personal = images.filter(i => i.type === 'personal').sort(() => Math.random() - 0.5);

// Interleave them
while(pro.length > 0 || personal.length > 0) {
  if (pro.length > 0) shuffled.push(pro.pop());
  if (personal.length > 0) shuffled.push(personal.pop());
  if (personal.length > 0) shuffled.push(personal.pop()); // more personal than pro
}
...
for (let img of shuffled) {
  result += `  {
    ...
    alt: "${img.alt}",
    caption: "${img.caption}",
    className: "${img.class} h-[200px] md:h-full"
  },\n`;
}
result += "]\n";
console.log(result);
